...
async function getMRStatus(mrLink, groupId) {
    try {
        ...
        if (mrLink.includes("github.com") && group.tokens.github) {
            ...
            const match = mrLink.match(/gitlab\.com\/([\w-]+(?:\/[\w-]+)*)\/-\/merge_requests\/(\d+)/);
            if (!match) throw new Error("Invalid GitLab MR link");
        
            const [_, projectPath, mrNumber] = match;
            // ✅ Fix the API URL format
            apiUrl = `https://gitlab.com/api/v4/projects/${encodeURIComponent(projectPath).replace(/%2F/g, "%2F")}/merge_requests/${mrNumber}`;
        
            headers = {
                "Authorization": `Bearer ${group.tokens.gitlab}`
            };
        }
